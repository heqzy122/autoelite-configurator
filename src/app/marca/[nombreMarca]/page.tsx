...
// ────────────────────────────────────
// Desactiva el chequeo de TS en este fichero
// @ts-nocheck
...
export async function generateStaticParams() {
  ...
  if (error) return [];
  const marcas = [...new Set(data.map((item: any) => item.marca))];
  return marcas.map((marca) => ({ nombreMarca: marca }));
}

export default async function MarcaPage(props: any) {
  const nombreMarca = decodeURIComponent(props.params.nombreMarca);
  ...
  console.log(`--- DEBUG: DATOS DEL SERVIDOR PARA MARCA -> ${nombreMarca} ---`);
  console.log(modelos);
  console.log("--------------------------------------------------");
